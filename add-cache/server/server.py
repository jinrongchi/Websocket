...
def main():
    ...
    print('Will wait for client / cache connections at port ' + str(server_socket.getsockname()[1]))
    server_socket.listen(1)
    
    # Keep the server running forever.
    
    while(1):
        print('Waiting for incoming client / cache connection ...')
        conn, addr = server_socket.accept()
        print('Accepted connection from client / cache address:', addr)
        print('Connection to client / cache established, waiting to receive message...')
        ...
        # Server now supports "If-modified-since" header
        # check whether this is a connection from web cache
        ...
        # Check the method and version first whether they are valid
        ...
        # We have the right request and version, so check if file exists.
            ...
            if (not os.path.exists(req_file)):
                ...
                # Check whether it is a conditional GET method from web cache
                # If the specified file does not need update, respond with a 304

                if headers_list[0] == 'If-modified-since':
                    # Get the modified time for the given file
                    # Used to check whether the file in cache location is the lasted version
                    modifiedTime = datetime.datetime.fromtimestamp(os.path.getmtime(req_file))

                    # Get the modified time for the file from cache
