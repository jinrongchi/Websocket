...
def main():
    ...
    # Connect to the load balancer for assigning server
    ...
    # Get the server information from the load balancer response

    response_line = get_line_from_socket(client_socket)
    print(response_line);

    header_line = get_line_from_socket(client_socket)
    print("Redirect to " + header_line)
    host = header_line.split("/")[2].split(":")[0]
    port = int(header_line.split("/")[2].split(":")[1])
